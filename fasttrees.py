...
class FastFrugalTreeClassifier(BaseEstimator, ClassifierMixin):
    ...
    def _predict_all(self, X: pd.DataFrame, cue_df: pd.DataFrame) -> pd.Series:
        ...
        def prediction_func(row):
            ...
            """
            ret_ser = pd.Series([],dtype=pd.StringDtype())
    ...
    def fit(self, X: pd.DataFrame, y: pd.DataFrame) -> FastFrugalTreeClassifier:
        ...
        if len(unique_labels(y)) > 2:
            raise ValueError(f'Fast and frugal trees are binary classification models, but ' f'the training data has the {len(unique_labels(y))} labels ' f'{unique_labels(y)}')
    ...
    def _validate_hyperparameters(self) -> None:
        ...
        if not isinstance(self.construction_algorithm, str):
            raise ValueError(f'construction_algorithm is not a str, ' f'got type {type(self.construction_algorithm)}')

        if not isinstance(self.scorer, Callable):
            raise ValueError(f'scorer must be a callable, ' f'got type {self.scorer}')

        if not isinstance(self.max_levels, int):
            raise ValueError(f'max_levels must be an int, ' f'got type {type(self.max_levels)}')
        ...
        if self.construction_algorithm not in self._construction_algorithms:
            raise ValueError(f'Construction algorithm {self.construction_algorithm} is not ' f'supported. Supported construction algorithms are ' f'{self._construction_algorithms}')
